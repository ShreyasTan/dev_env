...
Steps:
    1. Bring up UPF and SMF nodes.
    2. Start UPF node.
    3. Capture packets in SMF nodes.
...
class UPFTest():
    ...
    def manage_script_services(self):
        ...
        self._log.info("Waiting {} seconds for PFCP association and Session Establishment between UPF amd SMF nodes.".format(TestCaseConstants.SLEEP_TIME))
        time.sleep(TestCaseConstants.SLEEP_TIME)
        self.smf_helper_obj.stop_services(True)
        self._log.info("Stopped pfcp_session configuration script and captured packets on SMF node: '%s'", self.smf_helper_obj.get_management_ip())
        self._log.debug("<")
